...
| `--ssh-bind ADDR` | `ssh -R` 在远端监听的地址 | `127.0.0.1` |
...
ssh -NT \
  ...
  -R 127.0.0.1:47954:127.0.0.1:47954 \
  -R 127.0.0.1:45058:127.0.0.1:45058 \
  <ssh-host>
```
...
- 默认端口只监听远端主机的 `127.0.0.1`。确实需要让局域网其它机器直连时，显式传入 `--ssh-bind 0.0.0.0`，并同时检查 SSH server 的 `GatewayPorts` 和防火墙规则。也可以保留默认监听，再用受控的防火墙规则和 `socat` 将指定端口映射到局域网地址，例如：
```bash
# 添加防火墙规则
