...
| [`netcat-transfer.sh`](./sh/netcat-transfer/netcat-transfer.sh) | 使用 `netcat` 压缩传输文件或目录，可显示进度，接收端自动解压 | [README](./sh/netcat-transfer/README.md) |
...
先看脚本说明，再执行具体命令。每个脚本都提供 `--help`，可以查看使用命令。
